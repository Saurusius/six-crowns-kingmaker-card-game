...
export class SixCrownsHome extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext() {
    const [catalog, collection, decks, boosterCredits, specialBoosterCredits, eventBoosterCredits] = await Promise.all([
      loadCardCatalog(),
      getCollection(),
      getCustomDecks(),
      getBoosterCredits(),
  ...
  async _onRender(context, options) {
    ...
    this.element.querySelector("[data-action='analytics']")?.addEventListener("click", () => {
      void run(async () => {
        const api = moduleApi();
        if (typeof api?.openAnalyticsDashboard !== "function") throw new Error("Le tableau d’équilibrage n’est pas disponible.");
        await api.openAnalyticsDashboard();
      }, "Impossible d’ouvrir le tableau d’équilibrage.");
    });
  }
