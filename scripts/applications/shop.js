...
export class SixCrownsShop extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext(){
    ...
    return { crowns, products, pageTitle: this.tab === "inventory" ? "Votre réserve" : this.tab === "history" ? "Le registre" : "Ses nouvelles marchandises", shopTab:this.tab==="shop", inventoryTab:this.tab==="inventory", historyTab:this.tab==="history", history:history.map(e=>({...e,dateLabel:new Date(e.at).toLocaleString("fr-FR")})), hasInventory:products.some(p=>p.owned>0) };
  }
